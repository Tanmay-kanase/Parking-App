        ...
        >
          ...
          <div className="col-span-2 flex flex-wrap gap-4">
            {[
              {
                type: "car",
                icon: <FaCar className="text-2xl text-gray-600" />,
              },
              {
                type: "bike",
                icon: <FaMotorcycle className="text-2xl text-gray-600" />,
              },
              {
                type: "truck",
              ...
              {
                type: "EV",
                icon: <FaChargingStation className="text-2xl text-green-500" />,
              },
            ].map(({ type, icon }) => (
              <label
                key={type}
          ...
          {spots.map((spot, index) => (
            <div key={index} className="bg-white shadow-md rounded-xl p-6">
              ...
              <p className="text-gray-500 font-medium">
                Price: ${spot.pricePerHour} / hr
              </p>
              <p className="text-gray-500">Dimension: {spot.dimension}</p>

              <div className="flex gap-3 mt-3">
                {spot.vehicleTypes.includes("car") && (
                  <FaCar className="text-gray-600 text-xl" />
                )}
                {spot.vehicleTypes.includes("bike") && (
                  <FaMotorcycle className="text-gray-600 text-xl" />
                )}
                {spot.vehicleTypes.includes("truck") && (
                  <FaTruck className="text-gray-600 text-xl" />
                )}
                {spot.vehicleTypes.includes("EV") && (
                  <FaChargingStation className="text-green-500 text-xl" />
                )}
              </div>
              {spot.features.length > 0 && (
                <div className="mt-3">
                  ...
                  <ul className="text-gray-700 list-disc pl-5">
                    {spot.features.includes("camera_security") && (
                      <li>📷 Camera Security</li>
                    )}
                    {spot.features.includes("ev_charging") && (
                      <li>⚡ EV Charging</li>
                    )}
                    {spot.features.includes("washing_service") && (
                      <li>🧼 Washing Service</li>
                    )}
                    {spot.features.includes("valet_parking") && (
                      <li>🚗 Valet Parking</li>
                    )}
                  </ul>
                </div>
